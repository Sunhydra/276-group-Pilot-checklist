...
app.post('/api/saveChecklist',(req, res) => {
  let options = {
    root: __dirname + '/server-data/'
  };

    const fileName = 'checklist.json';
  let jsonFile = __dirname + '/server-data/checklistResult.json';
  fs.readFile(jsonFile,  async (err, data) => {
      ...
      let newId = Object.keys(results).count;
      let newRecord = {
        id: newId,
        result: req.body.form
      };
      results[newId] = newRecord;
      let db = JSON.stringify(results, null, 2);
      fs.writeFile(jsonFile, db, err => {
          if (err) {
              res.sendStatus(500);
              return;
          }
